...
function treeBoxes(urlService, jsonData)
{
	...
	// 1) ******************* Update the nodes *******************
	    ...
	    .attr('x', rectNode.width / 2 + tooltip.textMargin)
	    .attr('dy', '1.5em')
	    .text(function(d) {return 'Info: ' + d.info;});
		
		nodeEnterTooltip.append("text").append("tspan")
		.attr("class", "additionalInfo")
		.attr("dy", -5)
		.attr("dx", 2)
	    .text(function(d) {return 'More Info: ' + d.name;})
		.on("click", function(d) {
			console.log("click circle 2");
			$('#sidePanel').css('visibility', 'visible');
			$('#title').text(d.nodeName);
			$('#image').html('<img src="'+d.image+'" />');
			$('#infoPanel').html(d.additionalInfo);
		  });
